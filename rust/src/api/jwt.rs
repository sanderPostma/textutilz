        ...
        if !sec.is_empty() && parts.len() == 3 {
            ...
            if let Ok(hdr) = decode_header(&token) {
                ...
                let key_result = match hdr.alg {
                    ...
                    jsonwebtoken::Algorithm::RS256 | jsonwebtoken::Algorithm::RS384 | jsonwebtoken::Algorithm::RS512 | jsonwebtoken::Algorithm::PS256 | jsonwebtoken::Algorithm::PS384 | jsonwebtoken::Algorithm::PS512 => {
...
pub fn encode_jwt(header: String, payload: String, secret: String) -> Result<String, String> {
    ...
    let header_val: Value = serde_json::from_str(&header).map_err(|e| format!("Invalid Header JSON: {}", e))?;
    let payload_val: Value = serde_json::from_str(&payload).map_err(|e| format!("Invalid Payload JSON: {}", e))?;
...
mod tests {
    ...
    fn test_jwt_decode_es256() {
        ...
        println!("Invalid isValid: {}, error: {:?}", result.is_valid, result.error);
        assert!(!result.is_valid);
    }
}
